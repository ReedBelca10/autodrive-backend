...
$email = "test-$timestamp@autodrive.local"
...
Write-Host "`nNow logging in with $email..." -ForegroundColor Cyan
...
Write-Host "Login response status: $($loginResp.StatusCode)" -ForegroundColor Yellow

if ($null -eq $loginResp -or ($loginResp.StatusCode -ne 200 -and $loginResp.StatusCode -ne 201)) {
    Write-Host "Login failed!" -ForegroundColor Red
    if ($loginResp.Content) {
        Write-Host "Response: $($loginResp.Content)" -ForegroundColor Red
    }
    exit
}
...
$blogBody = @{
    ...
    excerpt = "Découvrez comment utiliser AutoDrive : réservations, profils, paiements et bien plus"
